...
def get_color_list(name, config, phase):
    ...
    image_root = os.path.join(config['data_path'], name, 'imgs')
    ...
    if config['stage'] > 1 and phase == 'train':
        ...
        gt_root = os.path.join(config['data_path'], name)
    ...
    for img_name in img_list:
        ...
        tag_dict['gt'] = os.path.join(image_root.replace('imgs','gts'), img_name.replace('jpg','png'))
...
class Test_Dataset:
    ...
    def load_data(self, index):
        ...
        try:
            ...
            gt = Image.open(img_name).convert('L')
